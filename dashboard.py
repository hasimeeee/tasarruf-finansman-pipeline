...
# ── KPI Kartları ────────────────────────────────────────────
# is_current = TRUE: sadece güncel snapshot (aktif/pasif/gecikmeli)
# Terk üyeler is_current=FALSE olduğu için churn hesabı ayrı sorgudan
...
kpi = query("""
    SELECT
        ...
        ROUND((100.0 * SUM(CASE WHEN m.member_status = 'aktif' THEN 1 ELSE 0 END)
              / NULLIF(COUNT(*), 0))::NUMERIC, 1)                                    AS aktif_oran,
...
col5.metric("🚪 Churn Oranı",      f"%{churn.iloc[0]['churn_orani']}")
col6.metric("🎰 Kura Oranı",       f"%{kura.iloc[0]['kura_orani']}")

st.markdown("---")

# ── Grafik 1: Aylık Tahsilat Trendi ─────────────────────────
# is_current filtresi YOK — terk üyelerin geçmiş ödemeleri dahil
...
# is_current filtresi YOK — tüm üyelerin plan dağılımı
with col_a:
    st.subheader("🏗️ Plan Tipi Dağılımı")
    plan_dagilim = query(f"""
        SELECT pl.plan_type, COUNT(DISTINCT p.member_key) AS uye_sayisi
        FROM fact_payments p
        JOIN dim_plan pl   ON p.plan_key = pl.plan_key
...
# is_current filtresi YOK — tüm geçmiş ödemeler
with col_b:
...
# is_current filtresi YOK — tüm geçmiş tahsilatlar
...
# is_current filtresi YOK — terk üyeler retention analizinin özü
